...
class Assault(
    ...
    override val type = EnemyType.ASSAULT
        ...
        if (currentState == GameplayState.Playing) {
            ...
            if (position != null) {
                if (direction == Direction.RIGHT) {
                    this.position = Point(this.position.x + SPEED, this.position.y + SPEED)
                } else {
                    this.position = Point(this.position.x - SPEED, this.position.y + SPEED)
                }
    ...
    companion object {
        const val SPEED = 1
        const val SCREEN_LEFT_OFFSET = 4
        const val SPRITE_WIDTH = 56.0
